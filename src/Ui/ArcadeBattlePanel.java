...
public class ArcadeBattlePanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    private final ArcadeEndOverlay endOverlay = new ArcadeEndOverlay();


    private final LeaderboardOverlay lbOverlay = new LeaderboardOverlay();

    // Repaint driver for overlays with animations
    private Timer overlayRepaintTimer;
    ...
    public ArcadeBattlePanel(int playerIndex, ArrayList<Integer> opponentOrder,
        ...
        overlayRepaintTimer = new Timer(16, e -> {
            if (endOverlay.isOpen() || lbOverlay.isOpen()) repaint();
        });
    ...
    private void loadAssets(String plHead, String opHead) {
        try { bgImage = new ImageIcon(loader.getResource("/backgrounds/cave.png")).getImage(); } catch (Exception e) {}
    ...
    private void stopAllTimers() {
        gameTimer.stop();
        secondTicker.stop();
        overlayRepaintTimer.stop();
        if (opponentDelayTimer != null) opponentDelayTimer.stop();
    }

    // ── End overlay callbacks ─────────────────────────────────────────────────

    private ArcadeEndOverlay.Callbacks endCallbacks() {
        return new ArcadeEndOverlay.Callbacks() {
    ...
    private void endFight() {
        ...
        new Timer(delay, e -> {
            ...
            if (!arcadeOver) {
                // Move to next fight immediately
                window.dispose();
                ...
                // Save score if won, then show in-canvas end overlay
                if (engine.stats1.hp > 0) {
                    ArcadeLeaderboard.addEntry(playerName,
                            factory.getName(playerIndex), elapsedSeconds);
                }
                showArcadeEndOverlay();
                // Restart the repaint timer so the overlay animates
                overlayRepaintTimer.restart();
            }
        }) {{ setRepeats(false); start(); }};
    }

    private void showArcadeEndOverlay() {
        boolean playerWon = engine.stats1.hp > 0;
        int hpPct = (int) Math.round(engine.stats1.hp * 100.0 / entities.BattleStats.MAX_HP);
        endOverlay.open(winner, playerWon, hpPct, formatTime(elapsedSeconds));
        repaint();
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (gameOver && !arcadeOver) drawGameOverOverlay(g2, sw, sh);

        // Pause menu
        if (paused) pauseMenu.draw(g2, sw, sh);

        if (endOverlay.isOpen()) endOverlay.draw(g2, sw, sh);
    ...
    public void mouseClicked(MouseEvent e) {
        // End overlay is topmost
        if (endOverlay.isOpen()) {
            endOverlay.handleClick(e.getPoint(), endCallbacks());
            repaint();
            return;
        }
        // Leaderboard overlay (standalone)
        if (lbOverlay.isOpen()) {
    ...
    public void mousePressed(MouseEvent e) {
        if (endOverlay.isOpen()) return;
        if (lbOverlay.isOpen()) return;
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (endOverlay.isOpen()) {
            endOverlay.handleHover(e.getPoint());
    ...
    public void keyPressed(KeyEvent e) {
        // End overlay intercepts all keys
        if (endOverlay.isOpen()) {
            endOverlay.handleKey(e.getKeyCode(), endCallbacks());
            repaint();
            return;
        }
        if (lbOverlay.isOpen()) {
