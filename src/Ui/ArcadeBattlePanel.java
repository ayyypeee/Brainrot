...
public class ArcadeBattlePanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    /** Carry-over constructor — HP is always reset to full, only timer state is preserved. */
    public ArcadeBattlePanel(int playerIndex, ArrayList<Integer> opponentOrder,
                             int currentOpponentPos, CharacterFactory factory,
                             int sw, int sh, Class<?> loader, String playerName,
        ...
        // HP intentionally NOT carried over — player always starts each fight at full HP
    }
    ...
    private void endFight() {
        ...
        new Timer(delay, e -> {
            if (!arcadeOver) {
                // Start next fight — player HP resets to full (BattleEngine constructor sets MAX_HP)
                window.dispose();
    ...
    private void showEscMenu() {
        ...
        Object[] options = { "Continue", "Main Menu" };
        int choice = JOptionPane.showOptionDialog(window, "Game Paused", "Pause",
    ...
    protected void paintComponent(Graphics g) {
        ...
        int hintFs = Math.max(10, (int)(sh * 0.016));
