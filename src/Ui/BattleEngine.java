...
public class BattleEngine {
    ...
    public void beginTurn(int side) {
        ...
        if (side == 1) {
            stats1.regenMana();
            stats2.regenMana();
            dialogueQueue.add("Round " + round + " begins! Both fighters regenerate 5 mana.");
    ...
    public int resolveSkill(int attackerSide, int skillNum) {
        ...
        if (attackerName.equals("AIP") && skillNum == 3) {
            ...
            dmg = attacker.computeSkill(skillNum, attackerName, defender, trueDmg);
        }
        ...
        if (selfHit) {
            ...
            announcement += " But it completely missed!";
        }
    ...
    public int applyPendingDamage() {
        ...
        if (selfHit) {
            if (dmg > 0) {
                attacker.addHp(-dmg);
                floatNumbers.add(new FloatNumber(dmg, pendingAttacker == 2));
            }
            selfHit = false;
        } else if (dmg > 0) {
            defender.addHp(-dmg);
            boolean defenderOnRight = (pendingAttacker == 1);
            floatNumbers.add(new FloatNumber(dmg, defenderOnRight));
        }
    ...
    public boolean canUseSkill(int side, int skillNum) {
        ...
        if (stats.hasEffect(StatusEffect.STUN) || stats.hasEffect(StatusEffect.HEAVY_STUN))
            return false;
        if (stats.hasEffect(StatusEffect.SILENCE)) return false;
    ...
    public String getVisibleText() {
        if (!dialogueActive || currentLine == null) return "";
        return currentLine.substring(0, charsRevealed);
    }
    public boolean isLineFinished() { return lineFinished; }
    public boolean hasQueuedLines() { return !dialogueQueue.isEmpty(); }

    private static String getFullCharacterName(String charName) {
        switch (charName) {
    ...
    public void drawHUD(Graphics2D g2, int sw, int sh, Component obs,
        ...
        if (tooltipText != null) {
            if (tooltipX >= hx1 && tooltipX <= hx1 + headSz
                    && tooltipY >= topY && tooltipY <= topY + headSz)
                drawCharTooltip(g2, name1, stats1, tooltipX, tooltipY, sw, sh);
            if (tooltipX >= hx2 && tooltipX <= hx2 + headSz
                    && tooltipY >= topY && tooltipY <= topY + headSz)
    ...
    private static class FloatNumber {
        ...
        FloatNumber(int dmg, boolean rightSide) {
            this.dmg = dmg;
    ...
    private void tickAndDrawFloatNumbers(Graphics2D g2, int sw, int sh) {
        ...
        for (FloatNumber fn : floatNumbers) {
            if (fn.y == 0) {
                fn.x = fn.rightSide ? sw * 0.72f : sw * 0.22f;
                fn.y = sh * 0.35f;
            }
            fn.y -= 1.5f;
            ...
            int fs = Math.max(22, (int)(sh * 0.06));
            g2.setFont(new Font(Font.MONOSPACED, Font.BOLD, fs));
            g2.setColor(new Color(0, 0, 0, fn.alpha / 2));
            g2.drawString("-" + fn.dmg, (int)fn.x + 2, (int)fn.y + 2);
            g2.setColor(new Color(255, 60, 60, fn.alpha));
            g2.drawString("-" + fn.dmg, (int)fn.x, (int)fn.y);
        }
    ...
    private void drawCharTooltip(Graphics2D g2, String charName, BattleStats stats,
        ...
        for (int i = 0; i < skills.length; i++) {
            ...
            String manaInfo;
            if (i == 0) {
                manaInfo = "[+" + sk.manaRegen + " MP]";
            } else {
                manaInfo = "[-" + sk.manaCost + " MP]";
            }
    ...
    public void drawSkillTooltip(Graphics2D g2, int side, int skillNum,
        ...
        String manaLine;
        if (skillNum == 1) {
            manaLine = "Mana: +" + skill.manaRegen + " (Regen)";
        } else {
            manaLine = "Mana: -" + skill.manaCost + " (Cost)";
        }
